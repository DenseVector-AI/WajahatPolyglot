...
PREPARED_DIR  = "./batch_all_input_500"              # where to write your batches
os.makedirs(PREPARED_DIR, exist_ok=True)

# Zero-based indices of lines to drop entirely:
...
def wrap_for_batch_chunk(lines_chunk, output_file_path, base_filename, start_idx_global=0, original_file_safe_name_for_id=""):
    ...
    Now uses the 'input' field as the source text.
    """
    ...
    with open(output_file_path, "w", encoding="utf-8") as fout:
        for local_idx, line in enumerate(lines_chunk):
            ...
            # 1) Drop only the explicitly listed bad lines:
            if global_idx in SKIP_LINE_INDICES:
                print(f"⚠️  Dropping line {global_idx} (in skip list)")
                continue

            # 2) Parse JSON (blank lines get an empty dict):
            if not raw:
                data = {}
            else:
                try:
            ...
            # 3) Extract the 'input' field only:
            input_text = str(data.get("input", "")).strip()

            # 4) Build your prompt around the input_text:
            combined_data_for_prompt = f"**Text**{input_text}\n"

            # — your same prompt template follows —
...
**Output:"""
            ...
            #  Build the OpenAI batch request
            request = {
